  ...
  async.parallel({
    ...
    serviceRequest: function checkServiceRequestDependency(next) {
      ...
      if (ServiceRequest) {
        ...
        ServiceRequest
          .count({ status: this._id }, function (error, count) {
...
StatusSchema.statics.findDefault = function (done) {
  ...
  Status.findOne().sort({ weight: 'asc' }, done); //TODO cache in memory
...
/* export status model */
module.exports = mongoose.model(STATUS_MODEL_NAME, StatusSchema);
