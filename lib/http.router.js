 ...
 * @apiSuccess {String} name Human readable name of the status 
 ...
 * which this status belongs
 * @apiSuccess {String} data.name Human readable name of the status 
 * e.g Open, In Progress, Resolved.
...
/* TODO apidoc */
router.get('/jurisdictions/:jurisdiction/statuses',
  function getStatuses(request, response, next) {

    //obtain request options
    const { jurisdiction } = request.params;
    const filter = (
      jurisdiction ? { filter: { jurisdiction: jurisdiction } } : {}
    ); //TODO support parent and no jurisdiction
    const options =
